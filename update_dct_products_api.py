#!/usr/bin/env python3
"""将 DCT 官网抓取结果合并进 minigt_products.json。"""
...
API_PRODUCTS_PATH = BASE_DIR / "dct_products_api.json"
...
CHANGED_KEYS = (
    ...
    "dct_url", "dct_title", "dct_categories", "dct_display_order", "dct_date",
)
...
def normalize_product(product):
    ...
    return normalize_catalog_product(normalized, key_label="DCT 产品", max_images=MAX_IMAGES_PER_PRODUCT, stringify_key=True)
...
def sort_product(product):
    return (int(product.get("dct_display_order") or 999999), str(product.get("sku") or ""))
...
def main():
    ...
    if not incoming:
        raise RuntimeError("dct_products_api.json 为空，停止合并")
    ...
    save_json(SUMMARY_PATH, {
        ...
        **merge_result_summary(
            result,
            original_count_key="original_dct_count",
            final_count_key="final_dct_count",
            extra={"max_images_per_product": MAX_IMAGES_PER_PRODUCT},
        ),
    })
    print(f"✅ DCT 合并完成：新增 {result['added_count']}，更新 {result['updated_count']}，未变 {result['unchanged_count']}")
