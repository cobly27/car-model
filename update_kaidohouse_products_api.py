#!/usr/bin/env python3
"""将 Kaido House diecast 接口结果合并进 minigt_products.json。"""
...
API_PRODUCTS_PATH = BASE_DIR / "kaidohouse_products_api.json"
SUMMARY_PATH = BASE_DIR / "kaidohouse_update_summary.json"
CATEGORY_ID = "kaidohouse"
CATEGORY_NAME = "Kaido House"
MAX_IMAGES_PER_PRODUCT = 4
CHANGED_KEYS = (
    "detail_id", "sku", "name", "status", "image", "images",
    "kaidohouse_url", "kaidohouse_handle", "kaidohouse_vendor",
    "kaidohouse_product_type", "kaidohouse_tags", "kaidohouse_display_order",
    "kaidohouse_published_at", "kaidohouse_updated_at", "kaidohouse_price",
    "kaidohouse_available", "kaidohouse_collection",
)
...
def sort_product(product):
    return (int(product.get("kaidohouse_display_order") or 999999), str(product.get("sku") or ""))
...
def main():
    ...
    if not incoming:
        raise RuntimeError("kaidohouse_products_api.json 为空，停止合并")
    ...
    save_json(SUMMARY_PATH, {
        ...
        **merge_result_summary(
            result,
            original_count_key="original_kaidohouse_count",
            final_count_key="final_kaidohouse_count",
            extra={"max_images_per_product": MAX_IMAGES_PER_PRODUCT},
        ),
    })
    print(f"✅ Kaido House 合并完成：新增 {result['added_count']}，更新 {result['updated_count']}，未变 {result['unchanged_count']}")
