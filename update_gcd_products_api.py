#!/usr/bin/env python3
"""将 GCD 官网抓取结果合并进 minigt_products.json。"""
...
API_PRODUCTS_PATH = BASE_DIR / "gcd_products_api.json"
...
CHANGED_KEYS = (
    ...
    "gcd_url", "gcd_title", "gcd_categories", "gcd_display_order", "gcd_date",
)
...
def normalize_product(product):
    ...
    return normalize_catalog_product(normalized, key_label="GCD 产品", max_images=MAX_IMAGES_PER_PRODUCT, stringify_key=True)
...
def sort_product(product):
    return (int(product.get("gcd_display_order") or 999999), str(product.get("sku") or ""))
...
def main():
    ...
    if not incoming:
        raise RuntimeError("gcd_products_api.json 为空，停止合并")
    ...
    save_json(SUMMARY_PATH, {
        ...
        **merge_result_summary(
            result,
            original_count_key="original_gcd_count",
            final_count_key="final_gcd_count",
            extra={"max_images_per_product": MAX_IMAGES_PER_PRODUCT},
        ),
    })
    print(f"✅ GCD 合并完成：新增 {result['added_count']}，更新 {result['updated_count']}，未变 {result['unchanged_count']}")
