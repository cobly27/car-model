#!/usr/bin/env python3
"""将 Tarmac Works Shopify 接口结果合并进 minigt_products.json。"""
...
API_PRODUCTS_PATH = BASE_DIR / "tarmacworks_products_api.json"
SUMMARY_PATH = BASE_DIR / "tarmacworks_update_summary.json"
CATEGORY_ID = "tarmacworks"
CATEGORY_NAME = "TARMAC WORKS"
MAX_IMAGES_PER_PRODUCT = 4
CHANGED_KEYS = (
    "detail_id", "sku", "name", "status", "image", "images",
    "tarmacworks_url", "tarmacworks_handle", "tarmacworks_vendor",
    "tarmacworks_product_type", "tarmacworks_tags", "tarmacworks_display_order",
    "tarmacworks_published_at", "tarmacworks_updated_at", "tarmacworks_price", "tarmacworks_available",
)
...
def normalize_product(product):
    ...
    return normalize_catalog_product(normalized, key_label="Tarmac Works 产品", max_images=MAX_IMAGES_PER_PRODUCT, stringify_key=True)
...
def sort_product(product):
    return (int(product.get("tarmacworks_display_order") or 999999), str(product.get("sku") or ""))
...
def main():
    ...
    if not incoming:
        raise RuntimeError("tarmacworks_products_api.json 为空，停止合并")
    ...
    save_json(SUMMARY_PATH, {
        ...
        **merge_result_summary(
            result,
            original_count_key="original_tarmacworks_count",
            final_count_key="final_tarmacworks_count",
            extra={"max_images_per_product": MAX_IMAGES_PER_PRODUCT},
        ),
    })
    print(f"✅ TARMAC WORKS 合并完成：新增 {result['added_count']}，更新 {result['updated_count']}，未变 {result['unchanged_count']}")
